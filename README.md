...
Set `cliff_generation.enabled` to `true` to automatically replace the entire top layer on steep slopes. The generator measures the steepest angle between each column and its immediate neighbours; if it exceeds `angle_threshold_degrees` the whole top-layer thickness swaps to the configured `cliff_generation.block`. Individual `biome_layers` entries can override the threshold and/or block via the optional `cliff_angle_threshold_degrees` and `cliff_block` keys, letting you keep snowy cliffs icy while lower elevations stay rocky.
